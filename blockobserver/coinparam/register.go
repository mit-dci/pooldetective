...
type Params struct {
	...
	Name string
	// Ticker is the market ticker for a coin
	Ticker string

	// CoinID is the numerical coin ID used in PoolDetective
	CoinID int
	...
	IdentifyAsClient  string
	IdentifyAsVersion string
}
...
func init() {
	...
	mustRegister(&BitcoinSVParams)
	mustRegister(&LitecoinCashParams)
	mustRegister(&LitecoinParams)
	mustRegister(&VertcoinParams)
	mustRegister(&BitcoinGoldParams)
}
