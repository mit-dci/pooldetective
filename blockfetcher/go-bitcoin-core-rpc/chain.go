...
import (
	...
	"github.com/mit-dci/pooldetective/blockfetcher/go-bitcoin-core-rpc/btcjson"
)
...
func (c *Client) GetBlockVerboseOld(blockHash *chainhash.Hash) (*btcjson.GetBlockVerboseResult, error) {
	return c.GetBlockVerboseOldAsync(blockHash).Receive()
}
...
//
// See GetBlockVerbose for the blocking version and more details.
func (c *Client) GetBlockVerboseOldAsync(blockHash *chainhash.Hash) FutureGetBlockVerboseResult {
	hash := ""
	if blockHash != nil {
		hash = blockHash.String()
	}

	cmd := btcjson.NewGetBlockCmd(hash, nil, nil)
	return c.sendCmd(cmd)
}
